  ...
  <div class="acl-layout">
    ...
    <div class="acl-right">
      <el-card>
        ...
        <rule-form :key="formKey" :submitting="submitting" @submit="addRule" />
      </el-card>
    </div>
  </div>
</template>
...
async function refresh() {
  const [snap, hits] = await Promise.all([
    getAclSnapshot().catch(() => ({ rules: [], version: 0, count: 0 })),
    getAclHits().catch(() => null)
  ])
